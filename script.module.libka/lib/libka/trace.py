            ...
            try:
                ...
                if isinstance(v, list):
                    if v and isinstance(v[0], str):
                        return list(s[:100] if isinstance(s, str) else s for s in v[:50])
                    return v[:50]
                return v
            except Exception:
...
def trace(func, details=TRACE_ALL, level=xbmc.LOGINFO):
    """Decorator for detailes trace."""
    @functools.wraps(func)
    def wrapped(*args, **kwds):
        tracer = TraceLogger(details=details, level=level)
        tracer.start()
        try:
...
def logthis(msg=''):
    ...
    log.info('{frame.f_code.co_filename}:{frame.f_lineno}: {msg}'.format(frame=frame, msg=msg))
